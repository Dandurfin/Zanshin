; *** Inno Setup - 简体中文 (Chinese Simplified) messages ***
;
; Preco je tento subor v projekte a nie v Inno Setupe: oficialna
; instalacia Inno Setup 6.5 cinstinu nedodava (je len medzi "user
; contributed" prekladmi na https://jrsoftware.org/files/istrans/).
; Hlasky, ktore tu chybaju, kompilator doplni z Default.isl (anglicky),
; takze subor nemusi byt uplny - ale sprievodca, tlacidla, odinstalovanie
; a bezne chyby su prelozene cele.
;
; Subor MUSI byt ulozeny ako UTF-8 s BOM (build_all.ps1 to kontroluje).
...
SetupFileMissing=安装目录中缺少文件 %1。请修正该问题或重新获取程序副本。
SetupFileCorrupt=安装文件已损坏。请重新获取程序副本。
SetupFileCorruptOrWrongVer=安装文件已损坏，或与此版本的安装程序不兼容。请修正该问题或重新获取程序副本。
...
InvalidPath=必须输入带盘符的完整路径，例如：%n%nC:\APP%n%n或如下形式的 UNC 路径：%n%n\\server\share
...
NoUninstallWarning=安装程序检测到以下组件已安装在您的计算机上：%n%n%1%n%n取消选择这些组件不会将其卸载。%n%n是否仍要继续？
ComponentSize1=%1 KB
ComponentSize2=%1 MB
...
SelectStartMenuFolderDesc=将程序快捷方式放在哪里？
SelectStartMenuFolderLabel3=安装程序将在以下开始菜单文件夹中创建程序快捷方式。
SelectStartMenuFolderBrowseLabel=单击“下一步”继续。若要选择其他文件夹，请单击“浏览”。
...
PreviousInstallNotCompleted=先前程序的安装/卸载尚未完成。需要重新启动计算机以完成该操作。%n%n重新启动后，请再次运行安装程序以完成 [name] 的安装。
CannotContinue=安装无法继续。请单击“取消”退出。
...
ErrorCloseApplications=安装程序无法自动关闭所有应用程序。建议在继续之前关闭所有正在使用需要更新的文件的应用程序。
PrepareToInstallNeedsRestart=安装程序必须重新启动您的计算机。重新启动后，请再次运行安装程序以完成 [name] 的安装。%n%n是否现在重新启动？
...
SelectDiskLabel2=请插入磁盘 %1 并单击“确定”。%n%n如果该磁盘上的文件位于下面显示以外的文件夹中，请输入正确路径或单击“浏览”。
PathLabel=路径(&P)：
...
UninstalledMost=%1 卸载完成。%n%n某些元素无法删除，可以手动删除。
UninstalledAndNeedsRestart=要完成 %1 的卸载，必须重新启动计算机。%n%n是否现在重新启动？
UninstallDataCorrupted=文件“%1”已损坏。无法卸载

; *** Uninstallation phase messages
ConfirmDeleteSharedFileTitle=删除共享文件？
ConfirmDeleteSharedFile2=系统指示以下共享文件已不再被任何程序使用。是否让卸载程序删除该共享文件？%n%n如果仍有程序在使用此文件而它被删除，这些程序可能无法正常工作。如果不确定，请选择“否”。将文件留在系统中不会造成任何危害。
